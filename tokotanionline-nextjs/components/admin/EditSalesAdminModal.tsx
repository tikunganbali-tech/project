 ...
 * 
 * Component: EditSalesAdminModal
 * 
 * Fungsi: Modal untuk edit sales admin
 */
...
interface EditSalesAdminModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  admin: SalesAdmin;
...
export default function EditSalesAdminModal({
  isOpen,
  onClose,
  onSuccess,
  admin,
}: EditSalesAdminModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'Failed to update sales admin');
        return;
      }
      ...
      setError('Failed to update sales admin');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 border-b">
          <h2 className="text-xl font-semibold text-gray-900">Edit Sales Admin</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'Updating...' : 'Update Sales Admin'}
            </button>
          </div>
        </form>
      </div>
